...
|---|---|---|
| Video materi gratis | 🔴 **Belum ada** | Tidak ditemukan satupun section "video gratis" di `bootcamp.html`/`bootcamp-join.html`/panel Bootcamp `dashboard.html`. Draf v1.0 dokumen ini salah menandainya ✅ — dikoreksi di sini. **Klarifikasi lanjutan 6 Sep 2026 (diskusi dengan pengguna):** ini juga **bukan** halaman/step video gratis terpisah sebelum bayar — maknanya adalah **video kelas ter-bundle di dalam Bootcamp Saya setelah bayar**, bukan lead-magnet pra-checkout. Ini sekaligus menjawab pertanyaan terbuka yang sudah lama ada di PRD §10 (baris ⚠️ sebelum tabel 10.1, sekarang sudah ditandai ✅ diklarifikasi). Detail desain: lihat §6.3 dan §11 Fase 2 di bawah. |
...
> ⚠️→✅ **Dampak ke §2 (diagram FigJam):** node "Video Gratis" sempat hilang total di diagram v2 (ditambal 5 Sep sebagai `Bootcamp Landing → Video Materi Gratis → Join WA`) — **tapi posisi tambalan itu SENDIRI salah**, ikut menganut Baca A (funnel pra-bayar) yang sudah dikoreksi. Ditambal ulang 6 Sep dengan posisi benar: node ini sekarang digambar **di dalam** kelompok "Bootcamp Saya (sudah bayar)", sejajar dengan Jadwal/Tugas/Leaderboard — bukan lagi di antara Landing dan Join.
...
- **(Fase 2, belum dibangun di Fase 1)** kartu ke-5 "Video Materi Gratis" — kartu kelas dari `CRYGLE_COURSES` yang termasuk dalam paket cohort ini, link ke `course-learning.html` (halaman yang sama dibangun Fase 1 §6.2), label "Termasuk Paket" menggantikan harga. Ditambahkan di sini sebagai catatan supaya tidak terlupa saat Fase 2, bukan bagian scope Fase 1
...
| **Fase 1** | Dashboard Peserta diperdalam: Overview kompleks (§6.1), Course Learning Hub (§6.2), Bootcamp Overview (§6.3) | **Koreksi:** bukan "semua data sudah ada" seperti draf v1.0 — mayoritas memang tinggal menata ulang UI (Peta Kompetensi, Progress Tugas, Peringkat), tapi 3 sub-bagian butuh kerja baru nyata: penyimpanan booking konsultasi (dasar untuk widget Jadwal Personal), field tanggal asli di data tugas (untuk widget Tenggat Terdekat), dan fitur attachment Bootcamp yang belum ada sama sekali (lihat detail di §6.1/§6.3). Tetap ROI tercepat dibanding Fase 2-5, cuma jangan dianggap "tanpa kerja backend baru" |
| **Fase 2** | Sistem Sertifikat (§7) **+ Video Materi Gratis Bootcamp** (§3 — **desain final diklarifikasi 6 Sep**: bukan halaman/funnel baru, tapi menampilkan kartu-kartu `CRYGLE_COURSES` yang relevan langsung di dalam Bootcamp Overview §6.3 dengan label "Termasuk dalam Paket Bootcamp — Akses Langsung" alih-alih harga, link ke `course-learning.html` yang sama dari Fase 1, bukan `checkout.html`) | Permintaan klien yang sudah lama tertunda (§3), sekaligus menjawab ambiguitas yang sudah lama terbuka di PRD §10. Jadi PRASYARAT alami untuk Fase 1 selesai terasa lengkap (Course Learning Hub butuh "apa yang didapat setelah selesai"). Video Gratis digabung di sini karena setelah diklarifikasi, effort-nya kecil (reuse `course-learning.html` dari Fase 1 + kartu kelas yang sudah ada) — tidak perlu fase sendiri |
| **Fase 3** | Mentor Portal (§8) | **Koreksi (samakan dengan angka final di §8):** bukan 80% reuse — 5 dari 9 layar reuse murah (Dashboard/Chat/Pendapatan/Profil), 3 layar (Kelola Materi, Cohort Roster, Booking) kerja baru nyata. Tetap ditaruh sebelum Admin Console karena porsi reuse-nya lebih tinggi & begitu Sertifikat ada, mentor perlu bisa lihat siapa yang sudah lulus dari sisi mereka |
